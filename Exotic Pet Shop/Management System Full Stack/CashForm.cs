    ...
    public partial class CashForm : Form
    {
        ...
        private void btnCash_Click(object sender, EventArgs e)
        {
            ...
            if(MessageBox.Show("Are you sure you want to cash this product?","Cashing",MessageBoxButtons.YesNo,MessageBoxIcon.Question)== DialogResult.Yes)
            {
                ...
                {
                    dbcon.executeQuery("UPDATE tblProducts SET pqty= pqty - " + int.Parse(dgvCash.Rows[i].Cells[4].Value.ToString()) + " WHERE pcode LIKE " + dgvCash.Rows[i].Cells[2].Value.ToString() + "");
                }
                dgvCash.Rows.Clear();
            }
        }
        ...
        private void dgvCash_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if(colName == "Delete")
            {
                if (MessageBox.Show("Are you sure you want to delete this item?", "Delete Transaction", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    dbcon.executeQuery("DELETE FROM tblCash WHERE cashid LIKE '" + dgvCash.Rows[e.RowIndex].Cells[1].Value.ToString() + "'");
                    MessageBox.Show("Transaction has been successfully removed!", title, MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
            else if (colName == "Increase")
            {
                ...
                {
                    dbcon.executeQuery("UPDATE tblCash SET qty = qty + " + 1 + " WHERE cashid LIKE '" + dgvCash.Rows[e.RowIndex].Cells[1].Value.ToString() + "'");
                }
                else
                {
            ...
            {
                if (int.Parse(dgvCash.Rows[e.RowIndex].Cells[1].Value.ToString()) == 1) 
                {
                    colName = "Delete";
                    goto removeitem;
                }
        ...
        public void loadCash()
        {
            try
            {
                ...
                double total = 0;
                dgvCash.Rows.Clear();
                cm = new SqlCommand("SELECT cashid,pcode,pname,qty,price,total,c.name,cashier FROM tblCash as cash LEFT JOIN tblCustomer c ON cash.cid = c.id WHERE transno LIKE " + lblTransno.Text + "", cn);
                cn.Open();
                dr = cm.ExecuteReader();
                while (dr.Read())
                {
        ...
        public int checkPqty(string pcode)
        {
            ...
            {
                cn.Open();
                cm = new SqlCommand("SELECT pqty FROM tblProducts WHERE pcode LIKE '" + pcode + "'", cn);
